...
export default function VideoCall() {
  ...
  const toggleMute = () => {
    setMuted(prev => {
      try { localRef.current?.srcObject?.getAudioTracks().forEach(t => t.enabled = prev); } catch(e) {}
      return !prev;
    });
  };
  ...
  const toggleVideo = () => {
    setVideoOff(prev => {
      try { localRef.current?.srcObject?.getVideoTracks().forEach(t => t.enabled = prev); } catch(e) {}
      return !prev;
    });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-3 bg-white rounded shadow">
        ...
        {status === 'Connecting...' && (
          <div className="mt-2 text-sm text-gray-600">
            Starting video call with {loc.state?.partner?.name || 'your partner'}...
          </div>
        )}
      </div>
      ...
      <div className="flex gap-3">
        <button className={`px-3 py-2 rounded ${muted ? 'bg-gray-400' : 'bg-indigo-600 text-white'}`} onClick={toggleMute}>
          {muted ? 'Unmute' : 'Mute'}
        </button>
        <button className={`px-3 py-2 rounded ${videoOff ? 'bg-gray-400' : 'bg-indigo-600 text-white'}`} onClick={toggleVideo}>
      ...
      <div className="mt-4 p-4 bg-white rounded shadow">
        ...
        <button className="mt-3 bg-indigo-600 text-white px-3 py-1 rounded" onClick={markCompleted}>
          Mark Completed
        </button>
      </div>
    </div>
